...
import { AddProjectCategoryComponent } from '../add-project-category/add-project-category.component';
...
export class ProjectCategoryListComponent implements OnInit, OnDestroy {
  ...
  loadCategories(): void {
    this.loading = true;
    this.apiService.getProjectCategories(
      this.pageIndex,
      this.pageSize,
      this.searchQuery,
      this.sortBy,
      ...
      error: (err: any) => {
        ...
        this.message.error('Failed to load Project Categories');
        this.categories = [];
        this.totalCount = 0;
      }
    });
  ...
  openEditDrawer(category: ProjectCategory): void {
  ...
  onStatusChange(category: ProjectCategory, newStatus: boolean): void {
    category.statusLoading = true;
    this.apiService.toggleProjectCategoryActiveStatus(category.id as string, newStatus).subscribe({
      next: () => {
  ...
  deleteCategory(id: string | number): void {
    this.loading = true;
    this.apiService.deleteProjectCategory(id).subscribe({
      next: () => {
