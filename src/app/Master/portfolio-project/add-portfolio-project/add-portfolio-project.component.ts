...
@Component({
  selector: 'app-add-portfolio-project',
  standalone: true,
  imports: [
    CommonModule,
    ReactiveFormsModule,
  ...
  templateUrl: './add-portfolio-project.component.html',
  styleUrl: './add-portfolio-project.component.css'
})
export class AddPortfolioProjectComponent implements OnChanges, OnInit {
  ...
  coverPreviews: { url: string, path: string }[] = [];
  ...
  get designStructure(): FormArray {
    return this.blogForm.get('designStructure') as FormArray;
  }

  addDesignStructureItem() {
    this.designStructure.push(this.fb.group({
      iconUrl: [''],
      label: [''],
      value: [''],
      isActive: [true]
  ...
  removeDesignStructureItem(index: number) {
    this.designStructure.removeAt(index);
  }

  onOverviewIconSelected(event: any, index: number) {
    const file = event.target.files[0];
    if (file && file.type.startsWith('image/')) {
      this.apiService.uploadImage(file, 'portfolioProjects').subscribe({
        next: (res: any) => {
          const url = res?.data?.url || res?.path || res?.url;
          if (url) {
            this.designStructure.at(index).patchValue({ iconUrl: url });
          }
        },
        error: (err) => {
  ...
  ngOnInit(): void {
    this.apiService.getCities(1, 100).subscribe({
      next: (res: any) => {
        let rawList: any[] = [];
        if (res) {
        ...
        if (rawList && rawList.length > 0) {
          this.citiesList = rawList.map((item: any) => {
            const cityName = item.name || item.Name || item.city || '';
            const stateName = item.state?.name || item.stateName || item.state || '';
            return {
    ...
    this.apiService.getProjectCategories(1, 100).subscribe({
      next: (res: any) => {
        let rawList: any[] = [];
        if (res) {
          if (Array.isArray(res)) {
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (changes['blogToEdit'] && this.blogToEdit) {
      this.blogForm.patchValue({
        ...
        fullDescription: this.blogToEdit.fullDescription || this.blogToEdit.shortDescription || this.blogToEdit.short_description,
        categoryId: this.blogToEdit.categoryId || this.blogToEdit.category_id,
        cityId: this.blogToEdit.cityId || null,
        year: this.blogToEdit.year || null,
        metaTitle: this.blogToEdit.metaTitle || '',
        metaDescription: this.blogToEdit.metaDescription || '',
        imageUrls: this.blogToEdit.imageUrls || this.blogToEdit.images || (this.blogToEdit.imageUrl ? [this.blogToEdit.imageUrl] : []),
      ...
      this.coverPreviews = [];
      const storedImages = this.blogToEdit.imageUrls || this.blogToEdit.images || (this.blogToEdit.imageUrl ? [this.blogToEdit.imageUrl] : []);
      if (storedImages && storedImages.length > 0) {
        storedImages.forEach((img: string) => {
          this.coverPreviews.push({ url: this.getAbsoluteImageUrl(img), path: img });
        });
      }
      
      this.designStructure.clear();
      const existingDesignStructure = this.blogToEdit.designStructure || this.blogToEdit.overviewItems;
      if (existingDesignStructure && Array.isArray(existingDesignStructure) && existingDesignStructure.length > 0) {
        existingDesignStructure.forEach((item: any) => {
          this.designStructure.push(this.fb.group({
        ...
        this.addDesignStructureItem(); // Add one blank item if none exist
      }
  ...
  resetForm(): void {
    ...
    this.sections.clear();
    this.designStructure.clear();
    this.addDesignStructureItem(); // Add one blank item by default
    this.sectionBuilderForm.reset({
      sectionTitle: '',
      sectionType: 'Paragraph',
      content: '',
  ...
  submitForm(): void {
    if (this.blogForm.valid) {
      ...
      const payload: any = {
        title: titleStr,
        slug: generatedSlug,
        fullDescription: rawValue.fullDescription,
        categoryId: rawValue.categoryId,
        cityId: rawValue.cityId,
        year: rawValue.year,
        metaTitle: rawValue.metaTitle,
        metaDescription: rawValue.metaDescription,
        imageUrls: rawValue.imageUrls || [],
        designStructure: rawValue.designStructure || [],
      ...
      if (this.blogToEdit) {
        // Update
        this.apiService.updatePortfolioProject(this.blogToEdit.id, payload).subscribe({
          next: () => {
            this.loading = false;
        ...
        // Create
        this.apiService.createPortfolioProject(payload).subscribe({
          next: () => {
            this.loading = false;
    ...
    if (files && files.length > 0) {
      ...
      for (let i = 0; i < files.length; i++) {
        ...
        if (!file.type.startsWith('image/')) {
          this.coverError = 'Please select valid image files only.';
          hasError = true;
          uploadedCount++;
          if (uploadedCount === files.length) this.uploadingCover = false;
          continue;
        }
        ...
        this.apiService.uploadImage(file, 'portfolioProjects').subscribe({
          next: (res: any) => {
            const url = res?.data?.url || res?.path || res?.url;
            if (url) {
              const currentImages = this.blogForm.get('imageUrls')?.value || [];
              this.blogForm.patchValue({ imageUrls: [...currentImages, url] });
              this.coverPreviews.push({ url: this.getAbsoluteImageUrl(url), path: url });
            }
            uploadedCount++;
          ...
          error: (err) => {
            hasError = true;
            this.coverError = 'Failed to upload some images.';
            console.error(err);
            uploadedCount++;
            if (uploadedCount === files.length) {
              this.uploadingCover = false;
  ...
  removeImage(index: number): void {
    ...
    if (Array.isArray(currentImages)) {
      ...
      this.blogForm.patchValue({ imageUrls: updatedImages });
    }
